...
As it can be observed the shape above is estimated to be `r coef(fit)[1]` and the scale is also estimated as `r coef(fit)[2]`. For the other distributions such as normal and weibull, their parameters can be estimated by tweaking the `dist` argument in the mle function.
