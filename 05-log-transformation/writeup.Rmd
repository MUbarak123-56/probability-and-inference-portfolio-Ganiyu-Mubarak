---
title: "Logartithmic Transformation"
...
arith_vs_geom <- function(d, var) {
  ggplot(d) +
    geom_point(aes(x = arithmetic, y = geometric), color = "blue") +
    geom_smooth(aes(x = arithmetic, y = geometric), method = "lm",
...
`E[log(X)]` is the mean of the log of X and `log(E[X])` is the log of the mean of X. The relationship between these two functions is that there are positive correlated that is as `log(E[X])` gets bigger `E[log(X)]`. One of the functions is not always bigger than the other. If `log(E[X])` is bigger than `E[log(X)]`, then that means `log[X]` has a negative non-linear relationship with `X`. If `E[log(X)]` is bigger than `log(E[X])`, then that means `log[X]` has a positive non-linear relationship with `X`. 
