...
`pba` is the probability that the Braves win if the game was to be played in the order NYC. It is calculated as 
...
calc2 <- function(adv) {
  ...
  d <- data.frame(g1 = c(), g2 = c(), g3 = c(), g4 = c(), g5 = c(),
                g6 = c(), g7 = c())
  set.seed(0)
  for (i in 1:5000) {
  ...
  d <- d %>% 
    ...
    mutate(win = if_else(count >= 4, "W", "L"))
  d_win <- d %>% 
    filter(win == "W")
