...
```
The first two images showcase how a playing session is bound to take place. The first one shows what is likely to happen if the player wins. The second one displays what a losing round looks like. The earnings are plotted on the y-axis while the wage number on the x-axis. According to the second image, it can be observed that a player using the Martingale's approach has a `r mean(walk_out_money > 200)*100`% chance of winning after 10000 trials, and the player is likely to lose about `r mean(walk_out_money - 200)` dollars. On average it takes about `r mean(play_length)` trials before stopping.
