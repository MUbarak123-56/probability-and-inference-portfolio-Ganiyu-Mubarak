...
The maximum likelihood estimate parameters for gh are `r mle_mean` and `r mle_sd` for mean and standard deviation respectively. The method of moments parameters for gh are `r mom_mean` and `r mom_sd` for mean and standard deviation respectively.
...
qplot(ht, dist="gamma", shape=mom_shape, scale = mom_shape, word2 = "mom")
```
...
qplot(ht, dist="weibull", shape=mom_shape, scale = mom_shape, word2 = "mom")
```
