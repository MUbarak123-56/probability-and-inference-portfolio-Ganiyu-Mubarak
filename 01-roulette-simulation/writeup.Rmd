...
x = 1:length(earning)
new_df <- data.frame(x = x, earning = earning)
ggplot(new_df) + geom_line(aes(x = x, y = earning), color = 'blue') + labs(title = sprintf("A typical round of a player losing a round of playing roulette after %d trials", length(earning)), x = "play number", y = "earning") + theme_bw()
...
```
The first two images showcase how a playing session is bound to take place. The first one shows what is likely to happen if the player wins. The second one displays what a losing round looks like. The earnings are plotted on the y-axis while the wage number on the x-axis. According to the second image, it can be observed that a player using the Martingale's approach has a 51% chance of winning after 10000 trials, and the player is likely to lose about 46 dollars. On average it takes about 200 trials before stopping.
...
Here the budget was set to 300 dollars, with the budget limit at 450 dollars and wager limit at 150 dollars. The probability of winning was about 50% and the expected earnings was -76 dollars.
...
Here the budget was set to 100 dollars, with the budget limit at 150 dollars and wager limit at 50 dollars. The probability of winning was about 53 % and the expected earnings was -20 dollars.
...
The Martingale Strategy was an amazing strategy to explore from a statistical standpoint because of it deploys statistical techniques in the real world. Martingale's theory about his strategy being error-proof was proven wrong because it was proven to only work **half of the time**. Any one who wants to play a game of roulette should take note of this information next time before playing. An improvement for this strategy will be to account for what happens when the green pocket is selected on the roulette wheel. Should the player get half of his wager back or should he get nothing? 
