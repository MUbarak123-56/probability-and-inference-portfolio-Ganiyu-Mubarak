...
d <- d %>% 
  ...
  select(-ID)
m <- median(sima)
new_val <- quantile(q, c(0.025, 0.975))
d <- d %>% 
  mutate(cover = if_else((low_end < m & high_end > m), 1, 0))
new_d <- d %>% 
  filter(cover == 1)
nrow(new_d)/nrow(d)
```
